...
However, for serious analyses, you may want to adjust the `iterations`, `burnin`, and `thin` slots to control the duration of the MCMC. `burinin` is the number of throwaway iterations at the beginning of the MCMC, `iterations` is the number of iterations for which parameter samples are actually used, and `thin` is the number of iterations to throw away between each actually-used iteration. You may also want to adjust the `Configs` slots that apply constraints on model parameters. The `parameter_sets_update` controls which sets of parameters to update during the MCMC. All other parameters are fixed at constant values. The `parameter_sets_return`, `genes_return`, `libraries_return`, `genes_return_epsilon`, and `libraries_return_epsilon` slots control which Monte Carlo parameter samples to return to the user.


### Assessing convergence with Gelman-Rubin potential scale reduction factors

If `diag` is set to `"gelman"` (default), Gelman-Rubin potential scale reduction factors will be used to assess convergence and then stored in the `psrf_all` and `psrf_important` slots of the subsequent `Chain` object. (For any other value of `diag`, no such convergence diagnostic will be used.). For more information on Gelman-Rubin potential scale reduction factors, see @bda.

Specifically, the MCMC will rerun the following procedure `max_attempts_diag` times or until all important potential scale reduction factors are less than `psrf_tol` ("important" meaning the hyperparameters and all $\beta_{\ell, g}$'s involved in contrasts). 

1. Run a pilot MCMC chain by itself for a fixed number of iterations determined by `burnin`, `thin`, and `iterations` (only done once). 
2. Initialize `nchains_diag - 1` new MCMC chains at starting values overdispersed relative to the joint posterior distribution estimated by the pilot chain.
3. Run each chain separately and compute Gelman-Rubin potential scale reduction factors of interest using all `nchains_diag` chains.
4. If all important potential scale reduction factors are less than `psrf_tol`, stop. Otherwise, multiply `burnin` and `thin` by 2 (or ad d 1 if 0). 


### Effective sample size calculations

If `diag` is `"gelman"`, extract the pilot chain at the end of the procedure in the previous section. Otherwise, there is only one chain to use. If the corresponding `ess` and `max_attempts_ess` slots are greater than 0, that MCMC chain will rerun `max_attempts_ess` times or until `ess` effective samples are obtained for each hyperparameter.
...
The `fbseq` function actually runs the MCMC algorithm to fit the model and calculate posterior probabilities of interest. It takes a single `Chain` object as an argument and outputs another `Chain` object with Gelman factors (if applicable), posterior means and means of squares of parameters, estimated posterior probabilities of interest, and updated starting values.
...
No actual output is shipped with the package because output  `Chain` objects are is usually large, in the tens of megabytes at least. However, all the functions to extract output are easy one-liners. I list them below in roughly decreasing order of usefulness. Let `chain` be a `Chain` object returned from `fbseq`.

|------------------------------------|----------|--------------------------------------------------| 
| `probs(chain)`   |	|  Estimated gene-specific posterior probabilities of each logical proposition specified in `Scenario` | 
| `volcano(chain)`   |	| `ggplot2` plot of posterior probabilities vs effect sizes for each proposition. | 
| `estimates(chain, level)`  |	|  Posterior means and credible intervals (of level `level`) for all parameters. Note: to avoid taking up too much memory, the credible intervals are computed using the posterior means and variances of parameters, not quantiles of actual parameter samples. The results are approximate. | 
| `effect_sizes(chain)`    |	| Estimated gene-specific effect sizes for each logical proposition. For example, if the proposition is "$2\beta_{2, g} - \beta_{4, g} > 1$ and $2\beta_{3, g} - \beta_{4, g} > 0.5$", then the effect size is $\min \left (2\beta_{2, g} - \beta_{4, g} - 1, \ 2\beta_{3, g} - \beta_{4, g} - 0.5 \right )^+$, where $(\cdots)^+$ denote the positive part of an expression. | 
|`flatten(chain)` |	$\qquad$ | Monte Carlo samples of returned parameters selected in `Configs` | 
|`flatten_post(chain)` |	$\qquad$ | Posterior means or means of squares of all or most of the parameters (configure with additional arguments) | 
