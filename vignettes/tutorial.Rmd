...
After installing [`fbseq`](https://github.com/wlandau/fbseq)  and [`fbseqSerial`](https://github.com/wlandau/fbseqSerial) , the following should take a couple seconds to run. The example walks through an example data analysis and shows a few key features of the package.
...
## Choosing a backend: [`fbseqSerial`](https://github.com/wlandau/fbseqSerial)  or [`fbseqCUDA`](https://github.com/wlandau/fbseqCUDA) 

[`fbseqSerial`](https://github.com/wlandau/fbseqSerial)  can run on most machines, but it is slow for datasets with lots of genes. Select the [`fbseqSerial`](https://github.com/wlandau/fbseqSerial)  backend by specifying `backend = "serial"` in [`fbseq`](https://github.com/wlandau/fbseq) :

```{r, eval = F}
out = fbseq(chain, backend = "serial")
```

For the serial backend, you can distribute the MCMC chains after the first chain over multiple parallel processes.
