...
generate_data = function(libraries = 12, genes = 3.5e4, 
          ...
          starts = Starts(nuGamma = 10, nuRho = 10, omegaSquared = 0.01, sigmaSquared = c(1, 0.25, 0.25), 
                                 tauGamma = 1, tauRho = 0.1, theta = c(3, 0, 0))){

  stopifnot(libraries >= 3)

  starts@xi = rep(1, ncol(design)*genes)
  starts@beta = numeric(0)
  for(l in 1:ncol(design))
    starts@beta = c(starts@beta, rnorm(genes, starts@theta[l], sqrt(starts@sigmaSquared[l])))
  
  starts@psi = rnorm(libraries, 0, sqrt(starts@omegaSquared))
  ...
  psimat = matrix(rep(starts@psi, each = genes), ncol = libraries)
  rhomat = matrix(rep(starts@rho, each = genes), ncol = libraries)
  gammat = matrix(rep(starts@gamma, times = libraries), ncol = libraries)
  epsilon = matrix(rnorm(libraries*genes, psimat, sqrt(rhomat*gammat)), ncol = libraries)
